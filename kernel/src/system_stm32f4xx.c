...
void SystemClockConfig(void) {
  ...
  // Set power scale 1 (requried for 168MHz)
  RCC->APB1ENR |= RCC_APB1ENR_PWREN;
  PWR->CR |= PWR_CR_VOS;

  // Configure Bus Prescalers
  // HCLCK = SYSCLK / 1 (168MHz)
  RCC->CFGR |= RCC_CFGR_HPRE_DIV1;

  // PCLK2 = HCLK / 2 (84Mhz max)
  RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;

  // PCLK1 = HCLK / 4 (42MHz max)
  RCC->CFGR |= RCC_CFGR_PPRE1_DIV4;

  // Configure PLL Factors (Assuming 8MHz HSE)
  // SYSCLK = (HSE / M) * N / P
  // Target 168MHz: (8 / 8) * 336 / 2 = 168
  uint32_t pllm = 8;
  uint32_t plln = 336;
  uint32_t pllp = 2; // encoded as 00
  uint32_t pllq = 7; // For USB 48MHz
  ...
  // 5 Wait States are required for 168MHz at 3.3V
